  ...
  return (
    <EditableSiteShell>
      <main className="mx-auto max-w-[1520px] px-4 py-10 sm:px-6 lg:px-8">
        <section className="rounded-[2.6rem] border border-[#6f4c2916] bg-[linear-gradient(180deg,#fffaf0,#f7ecd1)] p-6 shadow-[0_30px_90px_rgba(29,15,9,0.10)] sm:p-8">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="inline-flex items-center gap-2 text-xs font-black uppercase tracking-[0.24em] text-[#995f2f]">
            ...
            <button type="button" className="rounded-full border border-[#995f2f22] bg-white px-4 py-2 text-sm font-black text-[#622b14]" onClick={refreshComments}>Refresh comments</button>
          </div>

          <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="relative w-full sm:max-w-md">
              <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#995f2f]" />
              <input
                value={query}
                onChange={(event) => {
                ...
                className="h-11 w-full rounded-2xl border border-[#995f2f22] bg-white pl-9 pr-3 text-sm outline-none"
              />
            </div>
        ...
        {visibleComments.length ? (
          <section className="mt-8 grid gap-4">
            {visibleComments.map((item) => (
              <article key={`${item.articleSlug}-${item.id}`} className="rounded-[2rem] border border-[#6f4c2916] bg-white p-5 shadow-[0_16px_50px_rgba(29,15,9,0.08)]">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                  ...
                  {item.articleSlug ? (
                    <Link href={`/article/${item.articleSlug}`} className="text-sm font-black text-[#622b14] underline-offset-4 hover:underline">
                      Open article
                    </Link>
                  ) : null}
                </div>
          ...
          <section className="mt-8 rounded-[2rem] border border-dashed border-[#995f2f30] bg-white/75 p-8 text-center">
        ...
        {filtered.length > COMMENTS_PER_PAGE ? (
          <div className="mt-8 flex flex-wrap items-center justify-between gap-3 rounded-[2rem] border border-[#6f4c2916] bg-white p-4 text-sm text-[var(--slot4-muted-text)]">
            <span>Page {currentPage} of {totalPages}</span>
            <div className="flex gap-2">
              <button type="button" className="rounded-full border border-[#995f2f22] px-4 py-2 font-black disabled:opacity-40" disabled={currentPage <= 1} onClick={() => setPage((value) => Math.max(1, value - 1))}>Previous</button>
              <button type="button" className="rounded-full border border-[#995f2f22] px-4 py-2 font-black disabled:opacity-40" disabled={currentPage >= totalPages} onClick={() => setPage((value) => Math.min(totalPages, value + 1))}>Next</button>
            </div>
          </div>
        ) : null}
      </main>
